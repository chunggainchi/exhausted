...
export default function GamePage({ trackingVal, calibMin, calibMax, mediaStream, ghostImage, onReset, userName }: GamePageProps) {
  ...
  const takeSnapshot = (finalScore: number) => {
    ...
    if (ctx) {
        ...
        const displayName = userName ? userName.toUpperCase() : "DIBH DIVER";
  ...
  useEffect(() => {
    ...
    const loop = () => {
      ...
      if (!state.isGameOver) {
          ...
          // Base speed 3 for 800px width
          const widthFactor = width / 800; 
          const baseSpeed = 8 * (width < 600 ? 1.5 : 1); // Faster pixel traversal on small screens relative to size
          state.speed = baseSpeed * Math.pow(1.1, difficultyLevel);

          // Spawn Obstacles
          if (state.frameCount >= state.nextSpawnFrame) {
              // Gaps depend on height (so it's playable on portrait)
              const baseGap = height * 0.3; // 30% of screen height
              const minGap = 120;
              ...
              state.obstacles.push({
                  ...
                  gapTop,
                  gapHeight,
                  width: width * 0.1, // Obstacle width 10% of screen
                  passed: false
              });

              // Interval scales with speed/width
              const spawnInterval = Math.max(60, width / state.speed * 0.6); // Spacing approx 60% of screen width
          ...
          for (let i = state.obstacles.length - 1; i >= 0; i--) {
              ...
              // Collision Detection
              const diverRadius = Math.min(20, height * 0.03); // Scale diver size
              const diverXPos = width * 0.2; // Diver fixed at 20% width
              ...
              if (diverRight > obsLeft && diverLeft < obsRight) {
                  // Inside horizontal bounds of obstacle
      ...
      const scale = diverRadius / 15; // Base scale on radius
      ...
      {gameOver && (
        <div className="absolute inset-0 bg-black/90 z-50 animate-fade-in backdrop-blur-md flex flex-col items-center justify-center p-4">
            ...
            <p className="mt-3 md:mt-4 text-[#8da9c4] text-[10px] md:text-xs uppercase tracking-widest text-center">
                Challenge your friends to beat {score}m
            </p>
        </div>
      )}
      ...
      <div className="absolute bottom-2 left-2 right-2 md:bottom-4 md:left-4 md:right-auto md:w-[600px] h-20 md:h-32 bg-[#0d2b45]/90 border-2 border-[#203c56] shadow-xl p-2 flex gap-2 items-stretch rounded-sm z-10">
         ...
         <div className="flex-1 bg-black/50 border border-white/10 relative flex flex-col min-w-0">
             <div className="flex justify-between px-2 py-1 bg-[#203c56]/50">
                 <span className="text-[8px] md:text-[10px] text-[#4cc9f0] font-mono uppercase truncate">Bio-Signal</span>
             </div>
