...
import { AppStatus } from '@/components/dibh-diver/types';
...
export default function BreathingTracker({
  ...
  const matPrev = useRef<any>(null);
  const matCurr = useRef<any>(null);
  const point0 = useRef<any>(null);
  const point1 = useRef<any>(null);
  const winSize = useRef<any>(null);
  const criteria = useRef<any>(null);

  // --- INITIALIZATION ---
  useEffect(() => {
    const checkOpenCV = setInterval(() => {
      // Cast to any to avoid TypeScript errors during dynamic loading check
    ...
    return () => {
      ...
      try {
        ...
        if (winSize.current) winSize.current.delete();
        if (criteria.current) criteria.current.delete();
      } catch (e) { /* ignore */ }
    };
  }, []);
  ...
  if (!step1Complete) {
    videoOverlayText = "Align Head & Chest";
  } else if (!step2Complete) {
    videoOverlayText = "Click Zipper / Logo";
    overlayColor = "text-[#ff9f1c]";
  } else if (globalCalibMin === null) {
    videoOverlayText = "Set 'Normal' Breath";
    overlayColor = "text-[#4cc9f0]";
  } else if (globalCalibMax === null) {
    videoOverlayText = "Set 'Deep' Breath";
    overlayColor = "text-[#4cc9f0]";
  } else {
              ...
              <div className="w-full relative bg-black rounded-lg border-2 md:border-4 border-[#203c56] shadow-2xl aspect-[4/3] overflow-hidden group shrink-0">
                  ...
                  <div className="absolute top-2 left-2 z-30 pointer-events-none">
                      <div className={`px-2 py-1 text-[10px] md:text-sm font-black uppercase border-l-4 shadow-lg backdrop-blur-md
                        ...
                        {status === AppStatus.TRACKING ? 'TRACKING' : (status === AppStatus.READY ? 'SEARCHING' : 'LOST')}
                      </div>
                  </div>
              </div>
